...
function createWindow(): void {
  ...
  mainWindow = new BrowserWindow({
    ...
    minWidth: 800,
    minHeight: 700,
    title: "Starlink — Starlink Companion Desktop App (Unofficial)",
...
function createTray(): void {
  ...
  tray.setToolTip("Starlink");
  const menu = Menu.buildFromTemplate([
    { label: "Open Starlink", click: showWindow },
    { type: "separator" },
    {
...
function undeliverableReason(): string {
  return app.isPackaged
    ? "macOS isn’t delivering notifications — allow Starlink under System Settings ▸ Notifications."
    : "Native notifications need the installed Starlink app; a dev run can’t post them.";
}
