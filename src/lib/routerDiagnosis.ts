...
function messagesFor(routerAddress: string): Record<RouterUnreachableCause, string> {
  return {
    addressTaken:
      ...
      `${suggestedAlternative(routerAddress)}), or point Starlink's router address at wherever ` +
      `your Starlink router actually is.`,
    configuredAddressSilent:
      `Nothing answered at ${routerAddress}, the address Starlink is set to use, but the dish ` +
    ...
    differentNetwork:
      ...
      `Starlink's router address at where it is now.`,
    bypassed:
    ...
    unknown:
      ...
      `at an address other than the one Starlink is set to use.`,
    checking:
