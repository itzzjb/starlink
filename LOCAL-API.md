...
This is a whole-list update, not a single-client patch. The app therefore
...
a device present in the router's live client list. The app does not expose the
...
computer running the app, so both the extension UI capability and background
