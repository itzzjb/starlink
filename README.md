...
Ships as two things from one codebase: a **macOS**, **Windows** and **Linux**
desktop app, and a **browser extension** for Chrome, Edge and Firefox.
...
| Linux | `.AppImage`, `.deb`, `.rpm`, x64 + arm64 | `npm run pack:linux` |
| Chrome / Edge / Firefox | Extension | `npm run build:extension` |

Tagged `v*` pushes build all three desktop platforms and the extension into a
...
Linux packaging offers three formats because Linux has no single install
convention: AppImage runs anywhere with no package manager, `.deb` covers
Debian/Ubuntu via apt, `.rpm` covers Fedora/RHEL via dnf. Only the AppImage
self-updates; deb and rpm installs update through their package manager.
